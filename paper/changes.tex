...
%DIF ADD main.tex   Tue Nov 14 21:38:54 2023
...
\DIFaddbegin \DIFadd{A core assumption of our approach is that each participant organizes
their memories in a unique way. We defined each participant's }\textit{\DIFadd{memory
...
different lists, should be more similar than the estimated fingerprints computed
for different participants. We reasoned that the feature rich condition would
provide the best opportunity to test this assumption, since the clustering
scores would not be potentially confounded by order manipulations. To test our
``unique memory fingerprint'' assumption, we compared the similarity
(correlation) between the fingerprint from a single list (from one participant)
and (a) the average fingerprint from all other lists from the same participant
versus (b) the average fingerprint from each other participant (across all of
their lists). We found that participants' fingerprints on a held-out list are
...
\DIFaddend We also wondered whether adding these incidental visual features to later lists
...
[-2.461, 1.645]$}\DIFaddend ; reduced, word length: \DIFdelbegin \DIFdel{$t(60) = 3.528, p = 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$t(60) = 3.762, p < 0.001, d = 0.261,
...
p = 0.932$}\DIFdelend \DIFaddbegin \DIFadd{$t(40) = -0.101, p = 0.920, d = -0.009, \mathrm{CI} = [-2.307,
1.776]$}\DIFaddend ; reduced (late), size: \DIFdelbegin \DIFdel{$t(40) = 0.746, p = 0.460$}\DIFdelend \DIFaddbegin \DIFadd{$t(40) = 0.555, p = 0.582, d = 0.058,
\mathrm{CI} = [-1.444, 2.274] $}\DIFaddend ; reduced (late), length: \DIFdelbegin \DIFdel{$t(40) = 1.476, p = 0.148$}\DIFdelend \DIFaddbegin \DIFadd{$t(40) = 1.482, p =
0.146, d = 0.126, \mathrm{CI} = [-0.444, 3.743]$}\DIFaddend ; reduced (late), first letter:
\DIFdelbegin \DIFdel{$t(40) =
0.966, p = 0.340$}\DIFdelend \DIFaddbegin \DIFadd{$t(40) = -0.143, p = 0.887, d = -0.017, \mathrm{CI} = [-2.204, 1.830] $}\DIFaddend ). We
...
\DIFdelbegin \DIFdel{$< 1.013, p$s $> 0.314$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 1.013, p$s $\geq 0.314$}\DIFaddend ). Participants in both of the semantically
...
lists ($\|t\|$s \DIFdelbegin \DIFdel{$\leq 1.234, p$s $\geq 0.220$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 1.124, p$s $\geq 0.263$}\DIFaddend ).
