...
%DIF DEL old.tex    Tue Nov 14 21:41:53 2023
%DIF ADD main.tex   Tue Nov 21 12:01:58 2023
...
%DIF 24a28-31
\newcommand{\featureCorrs}{S9} %DIF > 
 %DIF > 
% tables %DIF > 
\newcommand{\abbreviations}{S1} %DIF > 
%DIF -------
...
\begin{abstract} \DIFdelbegin %DIFDELCMD < \footnotesize{We perceive, interpret, and remember ongoing
%DIFDELCMD < experiences through the lens of our prior experiences. Inferring that we are in
%DIFDELCMD < one type of situation versus another can lead us to interpret the same physical
%DIFDELCMD < experience differently. In turn, this can affect how we focus our attention,
%DIFDELCMD < form expectations about what will happen next, remember what is happening now,
%DIFDELCMD < draw on our prior related experiences, and so on. To study these phenomena, we
%DIFDELCMD < asked participants to perform simple word list-learning tasks. Across different
%DIFDELCMD < experimental conditions, we held the set of to-be-learned words constant, but
%DIFDELCMD < we manipulated how incidental visual features changed across words and lists,
%DIFDELCMD < along with the orders in which the words were studied. We found that these
%DIFDELCMD < manipulations affected not only how the participants recalled the manipulated
%DIFDELCMD < lists, but also how they recalled later (randomly ordered) lists. Our work
%DIFDELCMD < shows how structure in our ongoing experiences can influence how we remember
%DIFDELCMD < both our current experiences and unrelated subsequent experiences.
%DIFDELCMD < 

%DIFDELCMD < \textbf{Keywords: episodic memory, free recall, incidental features, implicit
%DIFDELCMD < priming, temporal order}}
%DIFDELCMD < %%%
...
experiences can take away very different meanings and memories. One reason \DIFaddbegin \DIFadd{for this }\DIFaddend is
...
lists~\citep{Kaha12, Kaha20}. In free recall \DIFaddbegin \DIFadd{paradigms}\DIFaddend , participants study lists of items
...
unstructured by design, it is not clear if \DIFdelbegin \DIFdel{, or how , }\DIFdelend \DIFaddbegin \DIFadd{or how }\DIFaddend non-trivial situation
models might apply to these stimuli. \DIFdelbegin \DIFdel{Nevertheless, there are }\textit{\DIFdel{some}}
%DIFAUXCMD
\DIFdel{commonalities between memory for word lists and memory for real-world
experiences}\DIFdelend \DIFaddbegin \DIFadd{As we unpack below, this provides an
important motivation for our current study, which uses free recall of
}\textit{\DIFadd{structured}} \DIFadd{lists to help bridge the gap between these two lines of
research}\DIFaddend .
...
early theories~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.][]{Este55a,AndeBowe72} }\hskip0pt%DIFAUXCMD
...
phenomena include the \DIFdelbegin \DIFdel{well known }\DIFdelend \DIFaddbegin \DIFadd{well-known }\DIFaddend recency and primacy effects (superior recall
of items from the end and, to a lesser extent, from the beginning of the
\DIFdelbegin \DIFdel{study
}\DIFdelend \DIFaddbegin \DIFadd{studied }\DIFaddend list), as well as semantic and temporal clustering
...
\DIFdelbegin \DIFdel{As formalized by }\DIFdelend \DIFaddbegin \DIFadd{According to }\DIFaddend models like the \textit{Context Maintenance and Retrieval\DIFdelbegin \DIFdel{Model}\DIFdelend }
\DIFaddbegin \DIFadd{model}\DIFaddend ~\citep{PolyEtal09}, the stimulus features associated with each word
(e.g.\DIFaddbegin \DIFadd{,}\DIFaddend \ the word's meaning, size of the object the word represents, \DIFdelbegin \DIFdel{the }\DIFdelend letters
that make up the word, font size, font color, location on the screen, etc.)\DIFaddbegin \DIFadd{~}\DIFaddend are
...
turn leads the participant to \DIFdelbegin \DIFdel{recall in
succession }\DIFdelend \DIFaddbegin \DIFadd{successively recall }\DIFaddend words that share stimulus
features.
...
experiences \DIFdelbegin \DIFdel{change }\DIFdelend \DIFaddbegin \DIFadd{changes }\DIFaddend across different real-world situations. We asked
...
two control conditions as a baseline; in these control conditions\DIFaddbegin \DIFadd{, }\DIFaddend all of the
...
additional eight lists whose items had the same features \DIFdelbegin \DIFdel{, but that }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend were sorted
in a random temporal order. We were interested in how these manipulations
\DIFdelbegin \DIFdel{affected }\DIFdelend \DIFaddbegin \DIFadd{might affect }\DIFaddend participants' recall behaviors on early (manipulated) lists, as well
as how order manipulations on early lists \DIFdelbegin \DIFdel{affected }\DIFdelend \DIFaddbegin \DIFadd{would affect }\DIFaddend recall behaviors on later
(randomly ordered) lists. Finally, in an \textit{adaptive} experimental
condition\DIFaddbegin \DIFadd{, }\DIFaddend we used participants' recall behaviors on \DIFdelbegin \DIFdel{early }\DIFdelend \DIFaddbegin \DIFadd{prior }\DIFaddend lists to manipulate,
in \DIFdelbegin \DIFdel{real-time}\DIFdelend \DIFaddbegin \DIFadd{real time}\DIFaddend , the presentation orders of subsequent lists. In this adaptive
condition, we varied \DIFdelbegin \DIFdel{the agreement between how participants }\DIFdelend \DIFaddbegin \DIFadd{whether the order in which items were presented agreed
or disagreed with how each participant }\DIFaddend preferred to organize their memories of
the studied items\DIFdelbegin \DIFdel{versus the orders in which the
items were presented.
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 

%DIF >  define the theoretical stakes of our study
\DIFaddbegin 

\DIFadd{From a theoretical perspective, we are interested in several core questions
...
}\textit{\DIFadd{future}} \DIFadd{experiences (which may or may not exhibit similar structure).
...
experimental conditions. The conditions included two controls (\DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend and
...
Participants \DIFdelbegin \DIFdel{either received }\DIFdelend \DIFaddbegin \DIFadd{received either }\DIFaddend course credit or a one-time \$10 \DIFaddbegin \DIFadd{cash }\DIFaddend payment for
...
education, vision, reading impairments, medications \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend recent injuries, coffee
...
A total of 318 participants reported their gender as female, 170 \DIFaddbegin \DIFadd{reported their
gender }\DIFaddend as male, and two participants declined to report their gender. A total
of 442 participants reported their ethnicity as ``not Hispanic or Latino,'' 39
\DIFaddbegin \DIFadd{reported their ethnicity }\DIFaddend as ``Hispanic or Latino,'' and nine declined to report
...
reading impairments\DIFdelbegin \DIFdel{, and
}\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend eight reported having mild reading impairments. A total of
489 participants reported having normal color vision and one participant
reported \DIFdelbegin \DIFdel{that they were
red-green color blind}\DIFdelend \DIFaddbegin \DIFadd{having impaired color vision}\DIFaddend . A total of 482 participants reported
...
concussion from several months prior. The participants reported \DIFdelbegin \DIFdel{consuming }\DIFdelend \DIFaddbegin \DIFadd{having consumed
}\DIFaddend 0--3 cups of coffee \DIFdelbegin \DIFdel{prior to the }\DIFdelend \DIFaddbegin \DIFadd{on the day of the }\DIFaddend testing session (mean: 0.32 cups;
...
study~\citep{ZimaEtal18}. \DIFdelbegin \DIFdel{The words all }\DIFdelend \DIFaddbegin \DIFadd{All words }\DIFaddend referred to concrete nouns \DIFdelbegin \DIFdel{, }\DIFdelend and were chosen
...
according to the approximate size of the object \DIFdelbegin \DIFdel{the word
}\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend referred to. Words were
labeled as ``small'' if the \DIFdelbegin \DIFdel{corresponding }\DIFdelend \DIFaddbegin \DIFadd{referent }\DIFaddend object was likely able to ``fit in a
...
per category: 1.33; standard deviation: 0.49). The \DIFdelbegin \DIFdel{numbers }\DIFdelend \DIFaddbegin \DIFadd{number }\DIFaddend of words in each
semantic category also varied from 12--28 (mean number of words per category:
17.07; standard deviation\DIFdelbegin \DIFdel{number of words}\DIFdelend : 4.65). We also identified lexicographic features for
each word, including \DIFdelbegin \DIFdel{the words'
first letters and lengths }\DIFdelend \DIFaddbegin \DIFadd{its first letter and length }\DIFaddend (i.e., number of letters).
Across all categories, all possible first letters were represented except for
`Q' (average number of unique first letters per category: \DIFdelbegin \DIFdel{11}\DIFdelend \DIFaddbegin \DIFadd{11.00}\DIFaddend ; standard
deviation: \DIFdelbegin \DIFdel{2 }\DIFdelend \DIFaddbegin \DIFadd{2.00 }\DIFaddend letters). Word lengths ranged from 3--12 letters (average: 6.17
letters; standard deviation: 2.06 letters).

We assigned the categorized words into a total of 16 lists with several
constraints. First, we required that each list \DIFdelbegin \DIFdel{contained words from }\DIFdelend \DIFaddbegin \DIFadd{contain }\DIFaddend exactly four unique
\DIFdelbegin \DIFdel{categories, each with exactly four exemplars from each category}\DIFdelend \DIFaddbegin \DIFadd{words from each of four unique categories}\DIFaddend . Second, we required that \DIFdelbegin \DIFdel{(across all words on the list
) }\DIFdelend \DIFaddbegin \DIFadd{each list
contain }\DIFaddend at least one \DIFdelbegin \DIFdel{instance
of both object sizes were represented}\DIFdelend \DIFaddbegin \DIFadd{word representing each of the two object sizes (``small''
and ``large'')}\DIFaddend . On average, each category was represented in 4.27 lists
(standard deviation: 1.16 lists). Aside from these two constraints, we \DIFaddbegin \DIFadd{randomly
}\DIFaddend assigned each word to a \DIFdelbegin \DIFdel{unique list }\DIFdelend \DIFaddbegin \DIFadd{single list (i.e., such that no words appeared in
multiple lists or were omitted entirely)}\DIFaddend . After random assignment, each list
contained words with an average of 11.13 unique starting letters (standard
deviation: 1.15 letters) and an average \DIFdelbegin \DIFdel{word }\DIFdelend length of 6.17 letters (standard
...
every condition\DIFaddbegin \DIFadd{, }\DIFaddend we randomized the study order of these lists across
...
word: the presentation font color \DIFdelbegin \DIFdel{, }\DIFdelend and the word's onscreen location. These
...
``reduced (late)'' condition. In these latter cases, \DIFdelbegin \DIFdel{words were all }\DIFdelend \DIFaddbegin \DIFadd{all words were }\DIFaddend presented
...
and at random from the interval \DIFdelbegin \DIFdel{$\left[0, 255\right]$}\DIFdelend \DIFaddbegin \DIFadd{$\left[0,~254\right]$}\DIFaddend , corresponding to the red
...
displays (resolution: 5120 \DIFdelbegin \DIFdel{$\times$ }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend 2880 pixels).
...
studied (which we call \DIFaddbegin \DIFadd{``}\DIFaddend \textit{early}\DIFaddbegin \DIFadd{'' }\DIFaddend lists) versus on the final eight
lists they studied (\DIFaddbegin \DIFadd{``}\DIFaddend \textit{late}\DIFaddbegin \DIFadd{'' }\DIFaddend lists). Since every participant studied
exactly 16 lists, every list was either ``early'' or ``late'' depending on its
order in the list study sequence. \DIFaddbegin \DIFadd{(In other words, the ``early'' and ``late''
...
Our experimental paradigm incorporates the Google Cloud \DIFdelbegin \DIFdel{Speech API
speech-to-text }\DIFdelend \DIFaddbegin \DIFadd{Speech-to-Text
}\DIFaddend engine~\citep{HalpEtal16} to automatically transcribe participants' verbal
...
baseline behaviors. We also used performance \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend these control conditions to
help interpret performance in other ``manipulation'' conditions. In the first
control condition, which we call the \textit{\DIFdelbegin \DIFdel{feature rich}\DIFdelend \DIFaddbegin \DIFadd{feature-rich}\DIFaddend } condition, we
...
rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend condition. However, rather than assigning each word a random color
and location, we instead displayed all of the words in black and at the center
of the screen.

We also designed two conditions \DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{in which }\DIFaddend we varied the words' visual
...
of the screen) for early lists, and followed the ``\DIFdelbegin \DIFdel{feature rich}\DIFdelend \DIFaddbegin \DIFadd{feature-rich}\DIFaddend '' procedure
(presenting each word in a random color and location) for late lists. Finally,
in the \textit{reduced (late)} condition, we followed the \DIFdelbegin \DIFdel{feature rich
}\DIFdelend \DIFaddbegin \DIFadd{feature-rich
}\DIFaddend procedure for early lists and the reduced procedure for late lists.
...
words' font colors and onscreen locations \DIFdelbegin \DIFdel{, }\DIFdelend as in the \DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend condition.
...
quantifying the dissimilarity between \DIFaddbegin \DIFadd{the values of that feature for }\DIFaddend each pair
of \DIFdelbegin \DIFdel{features}\DIFdelend \DIFaddbegin \DIFadd{words}\DIFaddend . This function varied according to the type of feature under
...
have a distance of \DIFdelbegin \DIFdel{1 for that feature.
}\DIFdelend \DIFaddbegin \DIFadd{1. }\DIFaddend Lexicographic features (length and first letter) are
\textit{discrete}. For these features\DIFaddbegin \DIFadd{, }\DIFaddend we defined a discrete distance function.
...
}\DIFdelend \DIFaddbegin \DIFadd{vectors and }\DIFaddend $(x, y)$ location vectors \DIFaddbegin \DIFadd{(specified as a percentage of the
viewable display's width)}\DIFaddend , respectively. Therefore, the color and location
distance measures always take on non-negative real values (upper-bounded at
\DIFdelbegin \DIFdel{441.67 }\DIFdelend \DIFaddbegin \DIFadd{439.94 }\DIFaddend for color, or \DIFdelbegin \DIFdel{27~in }\DIFdelend \DIFaddbegin \DIFadd{124.52 }\DIFaddend for location, reflecting the distances between the
...
chosen word's feature \DIFaddbegin \DIFadd{value}\DIFaddend (s) and the corresponding feature \DIFaddbegin \DIFadd{value}\DIFaddend (s) of all
yet-to-be-presented words. Third, we convert these distances (between the
previously presented word's feature values, $a$, and \DIFdelbegin \DIFdel{the candidate word' s
}\DIFdelend \DIFaddbegin \DIFadd{each of the $W$
yet-to-be-presented candidate words' }\DIFaddend feature values, \DIFdelbegin \DIFdel{$b$}\DIFdelend \DIFaddbegin \DIFadd{$b_{i \in 1...W}$}\DIFaddend ) to
similarity scores:

\begin{equation}
 \mathrm{similarity}(a, b\DIFaddbegin \DIFadd{_i}\DIFaddend ) = \exp\{-\tau \cdot
\mathrm{distance}(a, b\DIFaddbegin \DIFadd{_i}\DIFaddend )\}, \label{eqn:similarity}
\end{equation}
...
that this approach requires $\tau > 0$. Finally, we \DIFdelbegin \DIFdel{computed }\DIFdelend \DIFaddbegin \DIFadd{compute }\DIFaddend a set of
normalized similarity values by dividing the similarities by their sum:

\begin{equation}
\mathrm{similarity}_{\mathrm{normalized}}(a, b\DIFaddbegin \DIFadd{_i}\DIFaddend ) = \DIFdelbegin \DIFdel{\frac{\mathrm{similarity}(a,
b)}{\sum_{i=1}^n \mathrm{similarity}(a, i)}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\mathrm{similarity}(a,
b_i)}{\sum_{j=1}^W \mathrm{similarity}(a, b_j)}}\DIFaddend ,
\end{equation}
where in the denominator, \DIFdelbegin \DIFdel{$i$ takes on each of the $n$ feature values of }\DIFdelend \DIFaddbegin \DIFadd{$b_j$ takes on }\DIFaddend the \DIFaddbegin \DIFadd{feature value of each of the $W$
}\DIFaddend to-be-presented words. The resulting set of normalized similarity scores sums
to 1.
...
participants in the adaptive condition studied a total of 16 \DIFdelbegin \DIFdel{lists, }\DIFdelend \DIFaddbegin \DIFadd{word lists }\DIFaddend in a
randomized order. We varied the words' colors and locations for every word
presentation, as in the \DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend and order manipulation conditions.

All participants in the adaptive condition began the experiment by studying a
set of four \textit{initialization} lists. Words \DIFdelbegin \DIFdel{and features }\DIFdelend on these lists
...
participant's ``memory fingerprint,'' \DIFdelbegin \DIFdel{defined }\DIFdelend \DIFaddbegin \DIFadd{which we define }\DIFaddend below. At a high level, a
...
lists and in the \DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend condition). Lists in the stabilize and
...
considers one given feature dimension (e.g., category, color, etc.). We \DIFdelbegin \DIFdel{base
}\DIFdelend \DIFaddbegin \DIFadd{based
}\DIFaddend our main approach to computing clustering scores on analogous temporal and
...
sorted all not-yet-recalled words according to their absolute lag (\DIFdelbegin \DIFdel{that is, distance away in the }\DIFdelend \DIFaddbegin \DIFadd{i.e., their
distance from the just-recalled word in the presented }\DIFaddend list). We then computed
...
(for the given list and observed recall sequence), we \DIFdelbegin \DIFdel{compute }\DIFdelend \DIFaddbegin \DIFadd{constructed }\DIFaddend a ``null''
distribution of $n$ additional clustering scores \DIFdelbegin \DIFdel{after }\DIFdelend \DIFaddbegin \DIFadd{by repeatedly }\DIFaddend randomly
shuffling the order of the recalled words \DIFaddbegin \DIFadd{and recomputing the clustering score
for these shuffled recall sequences }\DIFaddend (we use $n = 500$ in the present study).
...
clustering scores (feature and temporal) reported in this paper. \DIFaddbegin \DIFadd{In
...
experiment (see \textit{Adaptive condition}) were sorted according to \DIFdelbegin \DIFdel{participants'}\DIFdelend \DIFaddbegin \DIFadd{each
individual participant's }\DIFaddend \textit{current} memory fingerprint, estimated using
...
noting\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend First, because memory fingerprints are computed independently for each
list and then averaged across lists, the already-computed memory fingerprints
for earlier lists could be cached and \DIFdelbegin \DIFdel{loaded }\DIFdelend \DIFaddbegin \DIFadd{retrieved }\DIFaddend as needed in future
computations. This meant that \DIFdelbegin \DIFdel{our computations pertaining to }\DIFdelend updating our estimate of a participant's memory
fingerprint \DIFdelbegin \DIFdel{only needed to consider data from the }\DIFdelend \DIFaddbegin \DIFadd{required computing feature and temporal clustering scores only for
the single }\DIFaddend most recent list. Second, \DIFdelbegin \DIFdel{each element of the null distributions of uncorrected
fingerprint scores (see }\textit{\DIFdel{Permutation-corrected feature clustering
scores}}%DIFAUXCMD
\DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{the clustering scores for each dimension
of a participant's memory fingerprint }\DIFaddend could be estimated independently from the
others\DIFdelbegin \DIFdel{. This enabled us to
...
}\DIFdelend \DIFaddbegin \DIFadd{, as could each element of the null distributions of uncorrected
clustering scores computed for each dimension (see
}\textit{\DIFadd{Permutation-corrected feature clustering scores}}\DIFadd{). This enabled us to
aggressively parallelize the fingerprint-updating procedure and }\DIFaddend compress the
...
Following some of our prior work~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{HeusEtal18a, HeusEtal21, MannEtal22}}\hskip0pt%DIFAUXCMD
, we use }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{FitzEtal23, HeusEtal18a, HeusEtal21,
MannEtal22}}\hskip0pt%DIFAUXCMD
, we used }\DIFaddend low-dimensional embeddings to help visualize how
...
matrix with number-of-lists (16) $\times$ number-of-conditions (10, \DIFdelbegin \DIFdel{encluding
}\DIFdelend \DIFaddbegin \DIFadd{excluding
}\DIFaddend the adaptive condition) rows and seven columns (one for each feature clustering
...
(Figs.~\ref{fig:fingerprint-trajectories}B,~\DIFdelbegin \DIFdel{and~}\DIFdelend \fingerprintTrajectoryRandom B), we
...
visualization\DIFdelbegin \DIFdel{. All }\DIFdelend \DIFaddbegin \DIFadd{; all }\DIFaddend statistical tests were carried out in the original
(seven-dimensional) feature spaces.

\DIFaddbegin \subsubsection*{\DIFadd{Factoring out the effects of temporal clustering}}

\DIFadd{For a given list of words, if the values along two feature dimensions (e.g.,
...
list. This means that the temporal clustering score will }\textit{\DIFadd{also}} \DIFadd{tend to
...
\DIFadd{We developed a permutation-based procedure to factor out the effects of
...
\DIFadd{While these temporally corrected clustering scores are useful for identifying
when feature clustering cannot be accounted for by temporal clustering alone,
they are }\textit{\DIFadd{not}} \DIFadd{necessarily valid estimates of the ``true'' degree to
...
its serial position during encoding. \DIFaddbegin \DIFadd{We used an analogous approach to compute
the proportion of trials on which each item (as a function of its presentation
position) was recalled at each output position $n$~\mbox{%DIFAUXCMD
\citep{Hoga75, HowaKaha99,
PolyEtal09, ZhanEtal23}}\hskip0pt%DIFAUXCMD
. }\DIFaddend To carry out this analysis, we initialized (for each
participant) a number-of-lists (16) by number-of-words-per-list (16) matrix of
0s. Then, for each list, we found the \DIFaddbegin \DIFadd{presentation }\DIFaddend index of the word that was
recalled first, and \DIFdelbegin \DIFdel{we }\DIFdelend filled in that position in the matrix with a 1. Finally, we
...
\DIFaddbegin \DIFadd{We note that several other studies have used a slightly different approach to
...
\citep[e.g.,][]{Farr10} }\hskip0pt%DIFAUXCMD
would be to count the item 1 column as
...
discussion of this issue in other work~\mbox{%DIFAUXCMD
\citep{MoraGosh14, Farr14}}\hskip0pt%DIFAUXCMD
.
}

\DIFaddend \subsubsection*{Lag-conditional response probability curve}
...
item, as a function of \DIFdelbegin \DIFdel{their }\DIFdelend \DIFaddbegin \DIFadd{the items' }\DIFaddend relative encoding positions (lag). In other
...
\DIFaddbegin \DIFadd{presentation positions of the }\DIFaddend just-recalled word \DIFdelbegin \DIFdel{'s presentation position }\DIFdelend and the next-recalled word\DIFdelbegin \DIFdel{'s presentation
position. We
}\DIFdelend \DIFaddbegin \DIFadd{. We
then }\DIFaddend computed the proportions of transitions (between successively recalled
...
\DIFdelbegin \DIFdel{successive }\DIFdelend repetitions (i.e., recalling \DIFdelbegin \DIFdel{the same word twice in a row}\DIFdelend \DIFaddbegin \DIFadd{a word that had already appeared in the current
recall sequence}\DIFaddend ). This yielded, for each list, a 1 by number-of-lags ($-15$ to
...
proportion of words at each position that the \DIFdelbegin \DIFdel{participant }\DIFdelend \DIFaddbegin \DIFadd{participants }\DIFaddend remembered.
...
feature dimension were greater than \DIFdelbegin \DIFdel{one standard deviation }\DIFdelend \DIFaddbegin \DIFadd{two standard deviations }\DIFaddend above the mean for
...
\DIFaddbegin \subsection*{\DIFadd{Transparency and openness}}

\DIFadd{All of the data analyzed in this manuscript, along with all of the code for
...
word-independent information might facilitate recall\DIFdelbegin \DIFdel{(e.g., by providing new potential ways of organizing or retrieving memories of the studied words) or impair recall(e.g., by
distracting participants with irrelevant information)}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[e.g., by providing
...
DrewMurd80, SochEtal09} }\hskip0pt%DIFAUXCMD
or impair recall~\mbox{%DIFAUXCMD
\citep[e.g., by distracting or
confusing participants with irrelevant information;][]{MarsEtal15, MarsEtal12,
Lang05, ReinEtal92}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Second, we manipulated the orders in which words were
...
participants organized their memories of the words\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[e.g., ][]{PolyKaha08,
MannEtal15}}\hskip0pt%DIFAUXCMD
}\DIFaddend . We also wondered whether some order manipulations might be
temporally ``sticky'' by influencing how \textit{future} lists were
remembered\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[e.g., ][]{SiroEtal05, LohnEtal10, Whit27, Badd68,
DarlMurd71}}\hskip0pt%DIFAUXCMD
}\DIFaddend .

\DIFaddbegin \begin{table}[tp]
\centering
\tiny
\input{tables/feature_rich_vs_reduced.tex}

\caption{\textbf{\DIFaddFL{Comparing memory in the feature-rich versus reduced conditions
(all lists).}} \DIFaddFL{The independent samples $t$-tests reported in the table were carried out
...
\DIFaddend To obtain a clean preliminary estimate of the consequences on memory of
randomly varying the font colors and locations of presented words (versus
holding the font color fixed at black, and \DIFdelbegin \DIFdel{holding the display }\DIFdelend \DIFaddbegin \DIFadd{the words' }\DIFaddend locations fixed at the
center of the \DIFdelbegin \DIFdel{display)}\DIFdelend \DIFaddbegin \DIFadd{screen), }\DIFaddend we compared participants' performance on the
\textit{\DIFdelbegin \DIFdel{feature rich}\DIFdelend \DIFaddbegin \DIFadd{feature-rich}\DIFaddend } and \textit{reduced} experimental conditions (see
\textit{Random conditions}, Fig.~\dynamicsRandom\DIFaddbegin \DIFadd{,
Tab.~\ref{tab:feature-rich-vs-reduced}}\DIFaddend ). In the \DIFdelbegin \DIFdel{feature rich
condition}\DIFdelend \DIFaddbegin \DIFadd{feature-rich condition, }\DIFaddend the
words' colors and locations varied randomly\DIFdelbegin \DIFdel{across words}\DIFdelend , and in the reduced condition\DIFaddbegin \DIFadd{,
}\DIFaddend words were always presented in black, at the center of the display. Aggregating
across all lists for each participant, we found no difference in recall
accuracy (i.e., the proportions of \DIFdelbegin \DIFdel{correctly recalled
words ) for feature rich }\DIFdelend \DIFaddbegin \DIFadd{words successfully recalled) for
feature-rich }\DIFaddend versus reduced lists\DIFdelbegin \DIFdel{($t(126) = -0.290, p = 0.772$)}\DIFdelend . However, participants in the \DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich
}\DIFaddend condition clustered their recalls substantially more along every dimension we
examined (\DIFdelbegin \DIFdel{temporal clustering:
...
7.775, p < 0.001$; }\DIFdelend see \textit{Permutation-corrected feature clustering scores} for more
...
features are independent of the words themselves and \DIFdelbegin \DIFdel{even when the
new features }\DIFdelend vary randomly across
...
\DIFdelbegin \DIFdel{We also wondered whether adding these incidental visual features to later lists
...
studied visually diverse lists ) might affect memory performance}\DIFdelend \DIFaddbegin \DIFadd{A core assumption of our approach is that each participant organizes their
memories in a unique way. We defined each participant's }\textit{\DIFadd{memory
fingerprint}} \DIFadd{as the set of their permutation-corrected clustering scores across
...
\caption{\textbf{\DIFaddFL{Comparing memory for early versus late lists in the
feature-rich condition.}} \DIFaddFL{The paired $t$-tests reported in the table were
...
\caption{\textbf{\DIFaddFL{Comparing memory for early versus late lists in the reduced
condition.}} \DIFaddFL{The paired $t$-tests reported in the table were carried out
...
\caption{\textbf{\DIFaddFL{Comparing memory for early versus late lists in the reduced
(early) condition.}} \DIFaddFL{The paired $t$-tests reported in the table were carried out
within-participant. Abbreviations used in this table are defined in
Table~\abbreviations.}}
...
\caption{\textbf{\DIFaddFL{Comparing memory for early versus late lists in the reduced
(late) condition.}} \DIFaddFL{The paired $t$-tests reported in the table were carried out
within-participant. Abbreviations used in this table are defined in
Table~\abbreviations.}}
...
\DIFadd{Beyond affecting participants' memories for individual lists, we wondered how
...
incidental visual features)}\DIFaddend . In other words, we sought to test for potential
...
\DIFdelbegin \DIFdel{feature rich and reduced
}\DIFdelend \DIFaddbegin \DIFadd{feature-rich (Tab.~\ref{tab:early-vs-late-feature-rich}) and reduced
(Tab.~\ref{tab:early-vs-late-reduced}) }\DIFaddend conditions. Participants in both
conditions remembered more words on early versus late lists\DIFdelbegin \DIFdel{(feature rich:
$t(66) = 4.553, p < 0.001$; reduced: $t(60) = 2.434, p = 0.018$)}\DIFdelend . Participants in
the \DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend (but not reduced) \DIFdelbegin \DIFdel{conditions }\DIFdelend \DIFaddbegin \DIFadd{condition }\DIFaddend exhibited more temporal
clustering on early versus late lists\DIFdelbegin \DIFdel{(feature rich: $t(66) = 2.318, p =
0.024$; reduced: $t(60) = 0.929, p = 0.357$)}\DIFdelend . And participants in both conditions
\DIFdelbegin \DIFdel{exhibited more semantic (category and size) }\DIFdelend \DIFaddbegin \DIFadd{tended to exhibit more semantic }\DIFaddend clustering on early versus late lists\DIFdelbegin \DIFdel{(feature rich, category: $t(66) = 3.805, p < 0.001$; feature
rich, size: $t(66) = 2.190, p = 0.032$; reduced, category: $t(60) = 2.856, p =
0.006$; reduced, size: $t(60) = 2.947, p = 0.005$)}\DIFdelend .
Participants in the reduced (but not \DIFdelbegin \DIFdel{feature rich) conditions exhibited }\DIFdelend \DIFaddbegin \DIFadd{feature-rich) conditions tended to exhibit
}\DIFaddend more lexicographic clustering on early versus late lists\DIFdelbegin \DIFdel{(feature rich, word length: $t(66) = 0.161, p = 0.872$;
feature rich, first letter: $t(66) = 0.410, p = 0.683$; reduced, word length:
$t(60) = 3.528, p = 0.001$; reduced, first letter: $t(60) = 2.275, p = 0.026$)}\DIFdelend . Taken together, these
...
participants tended to \DIFdelbegin \DIFdel{remember more words and }\DIFdelend exhibit stronger clustering effects on \DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich
}\DIFaddend (versus reduced) lists, and (b) participants tended to remember more words and
...
\DIFdelbegin \DIFdel{early versus late }\DIFdelend \DIFaddbegin \DIFadd{early-versus-late }\DIFaddend differences in accuracy\DIFdelbegin \DIFdel{(reduced (early): $t(41) =
1.499, p = 0.141$; reduced (late): $t(40) = 1.462, p = 0.152$)}\DIFdelend , temporal clustering\DIFdelbegin \DIFdel{(reduced (early): $t(41) = 0.998, p = 0.324$; reduced (late): $t(40)
= 1.099, p = 0.278$)}\DIFdelend , nor
feature-based clustering
(\DIFdelbegin \DIFdel{reduced (early),category:
$t(41) = 0.753, p = 0.456$; reduced (early), size: $t(41) = 0.721, p = 0.475$;
reduced (early), length: $t(41) = 0.493, p = 0.625$; reduced (early), first
letter: $t(41) = 0.780, p = 0.440$; reduced (late), category: $t(40) = -0.086,
p = 0.932$; reduced (late), size: $t(40) = 0.746, p = 0.460$; reduced (late),
length: $t(40) = 1.476, p = 0.148$; reduced (late), first letter: $t(40) =
0.966, p = 0.340$)}\DIFdelend \DIFaddbegin \DIFadd{Tabs.~\ref{tab:early-vs-late-early},~\ref{tab:early-vs-late-late})}\DIFaddend . We
...
lists\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[e.g.,][]{ClewEtal19,RadvZack17,RadvCope06}}\hskip0pt%DIFAUXCMD
}\DIFaddend . In prior work, we (and
...
\caption{\textbf{\DIFaddFL{Comparing memory in the feature-rich versus reduced (early)
conditions (all lists).}} \DIFaddFL{The independent samples $t$-tests reported in the
...
\caption{\textbf{\DIFaddFL{Comparing memory in the reduced versus reduced (early)
conditions (all lists).}} \DIFaddFL{The independent samples $t$-tests reported in the
table were carried out across-participants, and reflect data aggregated across
all lists from each participant. Abbreviations used in this table are defined
in Table~\abbreviations.}}
...
\caption{\textbf{\DIFaddFL{Comparing memory in the feature-rich versus reduced (late)
conditions (all lists).}} \DIFaddFL{The independent samples $t$-tests reported in the
table were carried out across-participants, and reflect data aggregated across
all lists from each participant. Abbreviations used in this table are defined
in Table~\abbreviations.}}
...
\caption{\textbf{\DIFaddFL{Comparing memory in the reduced versus reduced (late)
conditions (all lists).}} \DIFaddFL{The independent samples $t$-tests reported in the
table were carried out across-participants, and reflect data aggregated across
all lists from each participant. Abbreviations used in this table are defined
in Table~\abbreviations.}}
...
\DIFaddend We found that \textit{adding} incidental visual features on later lists that
...
(\DIFdelbegin \DIFdel{accuracy for feature rich versus reduced (early):
$t(107) = -2.230, p = 0.028$;
reduced versus reduced (early): $t(101) = -2.045,
p = 0.043$; }\DIFdelend \DIFaddbegin \DIFadd{Tabs.~\ref{tab:feature-rich-vs-reduced-early},~\ref{tab:reduced-vs-reduced-early};
}\DIFaddend also see Fig.~\accuracyByList A). However, \textit{subtracting} irrelevant
...
performance
(\DIFdelbegin \DIFdel{accuracy for feature rich versus reduced (late): $t(106) =
-0.638, p = 0.525$; reduced versus reduced (late): $t(100) = -0.407, p =
0.685$).}\DIFdelend \DIFaddbegin \DIFadd{Tabs.~\ref{tab:feature-rich-vs-reduced-late},~\ref{tab:reduced-vs-reduced-late})
}\DIFaddend These comparisons suggest that recall accuracy has a directional component:
accuracy is affected differently by removing features \DIFdelbegin \DIFdel{later
that had been present earlier }\DIFdelend \DIFaddbegin \DIFadd{that had initially been
present }\DIFaddend versus adding features \DIFdelbegin \DIFdel{later that had }\textit{\DIFdel{not}} %DIFAUXCMD
\DIFdel{been present earlier}\DIFdelend \DIFaddbegin \DIFadd{that had initially been absent}\DIFaddend . In contrast, we
found that participants exhibited more temporal and feature-based clustering
when we added incidental visual features to \textit{any} lists (\DIFdelbegin \DIFdel{comparisons of clustering on feature
rich versus reducedlists are reported above; temporal clustering in }\DIFdelend \DIFaddbegin \DIFadd{feature-rich
versus reduced: Tab.~\ref{tab:feature-rich-vs-reduced}; }\DIFaddend reduced versus reduced
(early)\DIFdelbegin \DIFdel{and reduced versus reduced (late) conditions: $t$s $\leq
-9.780$, $p$s $< 0.001$; feature-based clustering in }\DIFdelend \DIFaddbegin \DIFadd{: Tab.~\ref{tab:reduced-vs-reduced-early}; }\DIFaddend reduced versus reduced
(\DIFdelbegin \DIFdel{early) and reduced versus reduced (late)conditions: $t$s $\leq -5.443$, $p$s
$< 0.001$}\DIFdelend \DIFaddbegin \DIFadd{late}\DIFaddend )\DIFaddbegin \DIFadd{: Tab.~\ref{tab:reduced-vs-reduced-late})}\DIFaddend . Temporal and feature-based
clustering were not reliably different in the \DIFdelbegin \DIFdel{feature rich, reduced (early), and reduced (late) conditions (temporal
clustering in feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend versus reduced
(early) \DIFdelbegin \DIFdel{and feature rich versus
}\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend reduced (late) conditions
\DIFdelbegin \DIFdel{: $t$s $\geq -1.434$, $p$s $\geq 0.154$; feature-based
clustering in feature rich versus reduced (early) and feature rich versus
reduced (late) conditions: $t$s $\geq -1.359$,$p$s $> 0.177$}\DIFdelend \DIFaddbegin \DIFadd{(Tabs.~\ref{tab:feature-rich-vs-reduced-early},~\ref{tab:feature-rich-vs-reduced-late}}\DIFaddend ).
...
\caption{\DIFdelbeginFL \textbf{\DIFdelFL{Recall dynamics in feature rich free recall (order
manipulation conditions).}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Recall dynamics in feature-rich free recall (order
manipulation conditions).}} \DIFaddendFL \textbf{A.} Behavioral plots. \textbf{Left panels.}
...
of recalling each word are plotted as a function of the relative position (\DIFdelbeginFL \DIFdelFL{Lag}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lag}\DIFaddendFL )
to the \DIFdelbeginFL \DIFdelFL{words }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{word }\DIFaddendFL recalled just-prior. \textbf{Right panels.} The overall
probabilities of recalling each word are plotted as a function of presentation
position. \DIFdelbeginFL \textbf{\DIFdelFL{All panels.}} %DIFAUXCMD
\DIFdelFL{Error ribbons denote bootstrap-estimated 95\%
confidence intervals (calculated across participants). }\DIFdelendFL Top panels display the recall dynamics for early (order manipulation)
...
random and adaptive conditions. \textbf{B.} \DIFaddbeginFL \DIFaddFL{Distances between successively
...
position. See }\textit{\DIFaddFL{Defining feature-based distances}} \DIFaddFL{for additional
information. }\textbf{\DIFaddFL{C.}} \DIFaddendFL Proportion of event boundaries (see
\textit{Identifying event boundaries}) for each condition's feature of focus,
plotted as a function of presentation position. \DIFaddbeginFL \textbf{\DIFaddFL{All panels.}} \DIFaddFL{Error
ribbons denote bootstrap-estimated 95\% confidence intervals (calculated across
participants in Panel A, and across lists in Panels B and C).}\DIFaddendFL }
...
\caption{\textbf{\DIFaddFL{Comparing accuracy on early lists in the order manipulation
versus feature-rich conditions.}} \DIFaddFL{The independent samples $t$-tests reported in
...
\caption{\textbf{\DIFaddFL{Comparing temporal clustering on early lists in the order
manipulation versus feature-rich conditions.}} \DIFaddFL{The independent samples $t$-tests
...
\caption{\textbf{\DIFaddFL{Comparing feature-based clustering on early lists in the
semantic order manipulation versus feature-rich conditions.}} \DIFaddFL{The independent
...
\caption{\textbf{\DIFaddFL{Comparing feature-based clustering on early lists in the
lexicographic order manipulation versus feature-rich conditions.}} \DIFaddFL{The
...
\caption{\textbf{\DIFaddFL{Comparing feature-based clustering on early lists in the
visual order manipulation versus feature-rich conditions.}} \DIFaddFL{The independent
samples $t$-tests reported in the table were carried out across-participants,
and reflect data aggregated across all early lists from each participant.
Abbreviations used in this table are defined in Table~\abbreviations.}}
...
\DIFaddend Across each of six order manipulation conditions, we sorted early lists by one
...
letter, color, and location). \DIFdelbegin \DIFdel{Participants in the }\DIFdelend \DIFaddbegin \DIFadd{When we compared participants' memories for early
...
participants in the }\DIFaddend category-ordered \DIFdelbegin \DIFdel{condition
showed an increase in memory performance on early
lists (accuracy, relative to early feature rich lists ; $t(95) = 3.034, p = 0.003$). Participants in the
color-ordered condition
...
= 0.067$)}\DIFdelend \DIFaddbegin \DIFadd{condition remembered more words than
participants in the feature-rich condition}\DIFaddend . Participants' performances on early
lists in all of the other order manipulation conditions were indistinguishable
from performance on the early \DIFdelbegin \DIFdel{feature rich lists($\|t\|$s $< 1.013, p$s $> 0.314$). Participants in both of
the semantically ordered conditions exhibited stronger }\DIFdelend \DIFaddbegin \DIFadd{feature-rich lists. We also compared
participants' }\DIFaddend temporal clustering on early lists \DIFdelbegin \DIFdel{(versus early feature rich lists; category: $t(95) = 8.508, p <
0.001$; size: $t(95) = 2.429, p = 0.017$). Participants in the length-ordered
condition tended to
exhibit }\textit{\DIFdel{less}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{in each of these conditions to
their }\DIFaddend temporal clustering on early lists \DIFdelbegin \DIFdel{relative to early feature rich lists ($t(95) = -1.666, p = 0.099$), whereas
participants in
the }\DIFdelend \DIFaddbegin \DIFadd{in the feature-rich condition
(Tab.~\ref{tab:order-manip-vs-feature-rich-temporal-early}). Participants in
both of the semantically ordered conditions and the }\DIFaddend first letter-ordered
condition \DIFaddbegin \DIFadd{all }\DIFaddend exhibited stronger temporal clustering on early lists (\DIFdelbegin \DIFdel{$t(95) = 2.587, p = 0.011$}\DIFdelend \DIFaddbegin \DIFadd{vs.~early
feature-rich lists}\DIFaddend ). Participants in the \DIFdelbegin \DIFdel{visually ordered conditions
exhibited more similar performance }\DIFdelend \DIFaddbegin \DIFadd{other order manipulation conditions
all showed similar temporal clustering }\DIFaddend on early lists \DIFdelbegin \DIFdel{,
}\DIFdelend relative to early
\DIFdelbegin \DIFdel{feature rich lists(color: $t(96) = -1.064, p = 0.290$; we
found a trending enhancement for participants in the location-ordered
condition: $t(95) = 1.682, p = 0.096$)}\DIFdelend \DIFaddbegin \DIFadd{feature-rich lists}\DIFaddend . We also \DIFaddbegin \DIFadd{also }\DIFaddend compared feature-based clustering on early
lists across the order manipulation and \DIFdelbegin \DIFdel{feature rich
}\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend conditions. Since these
results were similar across both semantic conditions (category and size\DIFaddbegin \DIFadd{;
Tab.~\ref{tab:semantic-vs-feature-rich-feature-early}}\DIFaddend ), both lexicographic
conditions (length and first letter\DIFaddbegin \DIFadd{;
Tab.~\ref{tab:lexicographic-vs-feature-rich-feature-early}}\DIFaddend ), and both visual
conditions (color and location\DIFaddbegin \DIFadd{;
Tab.~\ref{tab:visual-vs-feature-rich-feature-early}}\DIFaddend ), here we aggregate data
...
to participants in the \DIFdelbegin \DIFdel{feature rich condition(category: $t(125) = 2.524, p =
0.013$; size: $t(125) = 3.510, p = 0.001$)}\DIFdelend \DIFaddbegin \DIFadd{feature-rich condition}\DIFaddend , but showed no reliable
differences in lexicographic \DIFdelbegin \DIFdel{(length: $t(125) = 0.539, p = 0.591$; first letter: $t(125) =
-0.587, p = 0.558$) or visual (color: $t(125) = -0.579, p = 0.564$; location:
$t(125) = -0.346, p = 0.730$) }\DIFdelend \DIFaddbegin \DIFadd{or visual }\DIFaddend clustering. Similarly, participants in
the lexicographically ordered conditions exhibited stronger (relative to
feature rich participants) lexicographic clustering \DIFdelbegin \DIFdel{(length: $t(125) = 3.426, p =
0.001$; first letter: $t(125) = 3.236, p = 0.002$) }\DIFdelend on early lists, but showed
no reliable differences in semantic \DIFdelbegin \DIFdel{(category: $t(125) = -1.078, p = 0.283$;
size: $t(125) = -0.310, p = 0.757$) or visual (color: $t(125) = -0.209, p =
0.835$; location: $t(125) = -0.004, p = 0.997$) }\DIFdelend \DIFaddbegin \DIFadd{or visual }\DIFaddend clustering. And participants in
the visually ordered conditions exhibited stronger visual clustering (again,
relative to \DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend participants, and on early lists\DIFdelbegin \DIFdel{; color: $t(126) =
2.099, p = 0.038$; location: $t(126) = 4.392, p < 0.001$}\DIFdelend ), but showed no
reliable differences in semantic \DIFdelbegin \DIFdel{(category: $t(126) = 0.204, p = 0.839$; size:
$t(126) = -0.093, p = 0.926$) or lexicographic (length: $t(126) = 0.714, p =
0.476$; first letter: $t(126) = 0.820, p = 0.414$) }\DIFdelend \DIFaddbegin \DIFadd{or lexicographic }\DIFaddend clustering. Taken together,
these order manipulation results suggest several broad patterns
(Figs.~\ref{fig:recall-dynamics}A,\DIFaddbegin \DIFadd{~}\DIFaddend \ref{fig:fingerprints}). First, most of the
...
manipulations enhanced visual clustering\DIFaddbegin \DIFadd{; Fig.~\ref{fig:barplots}C}\DIFaddend ) while
...
so on). \DIFaddbegin \DIFadd{Although it is not possible to fully separate feature-based versus
...
could be accounted for by temporal clustering alone (see }\textit{\DIFadd{Factoring out
the effects of temporal clustering}}\DIFadd{). When we carried out this analysis
...
across-participant \DIFdelbeginFL \DIFdelFL{distributions of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{average }\DIFaddendFL clustering scores for each feature type ($x$\DIFdelbeginFL \DIFdelFL{-coordinate}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-axis}\DIFaddendFL ) are
displayed for each experimental condition (color), separately for \DIFdelbeginFL \DIFdelFL{order manipulation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{order-manipulated
}\DIFaddendFL (early, top) and randomly ordered (late, bottom) lists. \DIFaddbeginFL \DIFaddFL{Error bars
denote bootstrap-estimated 95\% confidence intervals. }\DIFaddendFL See
...
\caption{\textbf{\DIFaddFL{Comparing accuracy on early lists in different order
manipulation conditions.}} \DIFaddFL{The independent samples $t$-tests reported in the
table were carried out across-participants, and reflect data aggregated across
...
\caption{\textbf{\DIFaddFL{Comparing temporal clustering on early lists in different
order manipulation conditions.}} \DIFaddFL{The independent samples $t$-tests reported in
the table were carried out across-participants, and reflect data aggregated
across all early lists from each participant. Abbreviations used in this table
are defined in Table~\abbreviations.}}
...
\caption{\textbf{\DIFaddFL{Comparing feature-based clustering on early lists in different
order manipulation conditions.}} \DIFaddFL{The independent samples $t$-tests reported in
the table were carried out across-participants, and reflect data aggregated
...
\DIFaddend The above comparisons between memory performance on early lists in the order
manipulation \DIFdelbegin \DIFdel{versus feature rich }\DIFdelend \DIFaddbegin \DIFadd{and feature-rich }\DIFaddend conditions highlight how sorted lists are
...
the other feature dimensions \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(accuracy:
...
clustering: Tab~\ref{tab:feature-comparisons-early}). }\DIFaddend Participants trended
towards remembering early lists that were sorted \DIFdelbegin \DIFdel{semantically better than lexicographically sorted lists
($t(118) = 1.936, p = 0.055$). Participants also remembered visually
sorted
lists }\DIFdelend \DIFaddbegin \DIFadd{visually
}\DIFaddend better than lexicographically sorted lists\DIFdelbegin \DIFdel{($t(119) = 2.145, p = 0.034$)}\DIFdelend . However, participants showed no
reliable differences in recall for semantically versus \DIFaddbegin \DIFadd{lexicographically or }\DIFaddend visually sorted lists\DIFdelbegin \DIFdel{($t(119) = 0.113, p = 0.910$)}\DIFdelend .
...
< 0.001$) sorted lists}\DIFdelend \DIFaddbegin \DIFadd{lists sorted either lexicographically or visually}\DIFaddend , but did not show reliable
differences in temporal clustering on lexicographically versus visually sorted
lists\DIFdelbegin \DIFdel{($t(119) = 0.189,
p = 0.850$)}\DIFdelend . Participants also showed reliably more semantic clustering on
semantically sorted lists than lexicographically \DIFdelbegin \DIFdel{(category: $t(118) = 3.492, p
= 0.001$, size: $t(118) = 3.972, p < 0.001$) or visually (category: $t(119) =
2.702, p = 0.008$, size: $t(119) = 4.230, p < 0.001$) }\DIFdelend \DIFaddbegin \DIFadd{or visually }\DIFaddend sorted lists; more
lexicographic clustering on lexicographically sorted lists than semantically \DIFdelbegin \DIFdel{(length: $t(118) = 3.112, p = 0.002$; first letter: $t(118) = 3.686, p <
0.001$) or
visually (length: $t(119) = 3.024, p = 0.003$; first letter: $t(119)
= 2.644, p = 0.009$) }\DIFdelend \DIFaddbegin \DIFadd{or
visually }\DIFaddend sorted lists; and more visual clustering on visually sorted lists than
semantically \DIFdelbegin \DIFdel{(color: $t(119) = -2.659, p = 0.009$; location:
$t(119) = -4.604, p < 0.001$) or lexicographically (color: $t(119) = -2.366, p
= 0.020$; location: $t(119) = -4.265, p < 0.001$) }\DIFdelend \DIFaddbegin \DIFadd{or lexicographically }\DIFaddend sorted lists. In summary, sorting lists by
...
\DIFdelendFL \DIFaddbeginFL \begin{sidewaysfigure}[p]
    ...
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\textwidth]{figures/memory_perf_barchart_compare}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/memory_perf_barchart_compare}
\DIFaddendFL 
...
probabilities (\textbf{A.}), temporal clustering scores (\textbf{B.}), \DIFdelbeginFL \DIFdelFL{and
}\DIFdelendFL feature
clustering scores (\textbf{C.})\DIFaddbeginFL \DIFaddFL{, and residual feature clustering scores (after
factoring out temporal clustering effects; }\textbf{\DIFaddFL{D.}}\DIFaddFL{) }\DIFaddendFL for early (gray) and
late (gold) lists. For the \DIFdelbeginFL \DIFdelFL{feature rich }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{feature-rich }\DIFaddendFL bars (left), the feature clustering
scores are averaged across \DIFdelbeginFL \DIFdelFL{features}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{all feature dimensions}\DIFaddendFL . For the order manipulation
...
rich condition. \DIFdelbeginFL %DIFDELCMD < \textbf%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The bars denote $t$-tests between the corresponding bars, and
the asterisks denote the Benjamini-Hochberg-corrected $p$-values. Comparisons
for which corrected $p \geq 0.05$ are not shown.}\DIFaddendFL } \label{fig:barplots}
...
\caption{\textbf{\DIFaddFL{Comparing accuracy on late lists in order-manipulation versus
feature-rich conditions.}} \DIFaddFL{The independent samples $t$-tests reported in the
table were carried out across-participants, and reflect data aggregated across
...
\caption{\textbf{\DIFaddFL{Comparing temporal clustering on late lists in
order-manipulation versus feature-rich conditions.}} \DIFaddFL{The independent samples
...
\caption{\textbf{\DIFaddFL{Comparing feature-based clustering on late lists in semantic
order-manipulation versus feature-rich conditions.}} \DIFaddFL{The independent samples
$t$-tests reported in the table were carried out across-participants, and
reflect data aggregated across all late lists from each participant.
Abbreviations used in this table are defined in Table~\abbreviations.}}
...
\caption{\textbf{\DIFaddFL{Comparing feature-based clustering on late lists in
lexicographic order-manipulation versus feature-rich conditions.}} \DIFaddFL{The
independent samples $t$-tests reported in the table were carried out
...
\caption{\textbf{\DIFaddFL{Comparing feature-based clustering on late lists in visual
order-manipulation versus feature-rich conditions.}} \DIFaddFL{The independent samples
$t$-tests reported in the table were carried out across-participants, and
reflect data aggregated across all late lists from each participant.
Abbreviations used in this table are defined in Table~\abbreviations.}}
...
\DIFaddend At the group level, there seemed to be almost no lingering impact of sorting
early lists on memory for later lists. To simplify the presentation, we report
these null results in aggregate across the three feature groupings \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(accuracy:
...
and~\ref{tab:visual-feature-carryover}). }\DIFaddend Relative to memory performance on late
\DIFdelbegin \DIFdel{feature rich }\DIFdelend \DIFaddbegin \DIFadd{feature-rich }\DIFaddend lists, participants' memory performance in all six order
manipulation conditions showed no reliable differences\DIFdelbegin \DIFdel{(semantic: $t(125) = 0.487, p = 0.627$; lexicographic: $t(125) = 0.878, p =
0.382$; visual: $t(126) = 1.437, p = 0.153$)}\DIFdelend . Nor did we observe any
reliable differences in temporal clustering on late lists (relative to late
\DIFdelbegin \DIFdel{feature rich
lists; semantic: $t(125) = 0.146, p = 0.884$; lexicographic: $t(125) = 0.923, p
= 0.358$; visual: $t(126) = 0.525, p = 0.601$}\DIFdelend \DIFaddbegin \DIFadd{feature-rich lists}\DIFaddend ). Aside from a slightly increased tendency for participants
to cluster words by their length on late visual order manipulation lists (more
than late \DIFdelbegin \DIFdel{feature rich lists; $t(126) = 2.199, p =
0.030$}\DIFdelend \DIFaddbegin \DIFadd{feature-rich lists}\DIFaddend ), we observed no reliable differences in any type
of feature clustering on late order manipulation condition lists versus late
\DIFdelbegin \DIFdel{feature rich lists($\|t\|$s $\leq 1.234, p$s $\geq 0.220$)}\DIFdelend \DIFaddbegin \DIFadd{feature-rich lists}\DIFaddend .
...
defined participants' }\textit{\DIFdel{memory fingerprints}} %DIFAUXCMD
\DIFdel{as the set of their temporal
and feature clustering scores (see }\textit{\DIFdel{Memory fingerprints}}%DIFAUXCMD
\DIFdel{). A
...
\begin{figure}[tp] \centering
    ...
    (across-participant) average memory fingerprint for \DIFdelbeginFL \DIFdelFL{one }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a single }\DIFaddendFL list. \DIFaddbeginFL \DIFaddFL{Lines
    connect successive lists. }\DIFaddendFL Order manipulation (early) lists are denoted by
    ...
    \DIFdelbeginFL \DIFdelFL{of the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{between each }\DIFaddendFL per-list memory \DIFdelbeginFL \DIFdelFL{fingerprints to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fingerprint and }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{list 0 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{average }\DIFaddendFL fingerprint \DIFaddbeginFL \DIFaddFL{across
    all prior lists}\DIFaddendFL , for each condition. Error bars denote bootstrap-estimated
...
across conditions and participants (early: \DIFdelbegin \DIFdel{$r(179) = 0.537, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.492,~p < 0.001,
\mathrm{CI} = [0.352, 0.606]$}\DIFaddend ; late: \DIFdelbegin \DIFdel{$
r(179) = 0.492, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.403,~p < 0.001,~\mathrm{CI} =
[0.271,~0.517]$}\DIFaddend ) as well as for each condition individually for early ($r$s
\DIFdelbegin \DIFdel{$\geq 0.386$}\DIFdelend \DIFaddbegin \DIFadd{$\geq 0.331$}\DIFaddend , all $p$s \DIFdelbegin \DIFdel{$\leq 0.035$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 0.069$}\DIFaddend ) and late ($r$s \DIFdelbegin \DIFdel{$\geq 0.462$}\DIFdelend \DIFaddbegin \DIFadd{$\geq 0.404$}\DIFaddend , all $p$s
\DIFdelbegin \DIFdel{$\leq 0.010$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 0.027$}\DIFaddend ) lists. We found no evidence of a condition-level trend; for
...
more words (early: \DIFdelbegin \DIFdel{$r(4) = 0.526, p = 0.284$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = 0.511,~p = 0.300,~\mathrm{CI} = [-0.999,~0.996]$}\DIFaddend ;
late: \DIFdelbegin \DIFdel{$r(4) = -0.257,
p = 0.623$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = -0.304,~p = 0.559,~\mathrm{CI} = [-0.833,~0.748]$}\DIFaddend ; see insets of
...
feature clustering on early lists \DIFaddbegin \DIFadd{in the non-visual conditions }\DIFaddend tended to recall
more items on late lists (across conditions: \DIFdelbegin \DIFdel{$r(179) = 0.492, p < 0.001$; all }\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.230,~p = 0.002,
\mathrm{CI} = [0.072,~0.372]$; all non-visual }\DIFaddend conditions individually: $r$s
\DIFdelbegin \DIFdel{$\geq 0.462$}\DIFdelend \DIFaddbegin \DIFadd{$\geq 0.405$}\DIFaddend , all $p$s \DIFdelbegin \DIFdel{$\leq 0.010$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 0.027$; color: $r(29) = 0.212,~p = 0.251,
\mathrm{CI} = [-0.164,~0.532]$; location: $r(28) = 0.320,~p = 0.085,
\mathrm{CI} = [0.011,~0.584]$}\DIFaddend ). Participants who recalled more items on early
lists also tended to show stronger feature clustering on late lists (across
conditions: \DIFdelbegin \DIFdel{$r(179) = 0.280, p < 0.001$;
}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.464,~p < 0.001,~\mathrm{CI} = [0.321,~0.582]$;
individual conditions: }\DIFaddend all \DIFdelbegin \DIFdel{non-visual conditions: }\DIFdelend $r$s \DIFdelbegin \DIFdel{$\geq 0.445$}\DIFdelend \DIFaddbegin \DIFadd{$\geq 0.377$}\DIFaddend , all $p$s \DIFdelbegin \DIFdel{$\leq 0.014$; color: $r(29)
= 0.298, p = 0.103$; location: $r(28) = 0.354, p = 0.055$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 0.040$}\DIFaddend ). Neither
of these effects showed condition-level trends (early feature clustering versus
late recall probability: \DIFdelbegin \DIFdel{$r(4) = -0.299, p = 0.565$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = -0.338,~p = 0.512,~\mathrm{CI} = [-0.971,~
0.634]$}\DIFaddend ; early recall probability versus late feature clustering: \DIFdelbegin \DIFdel{$r(4) = 0.400, p = 0.432$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) =
0.451,~p = 0.369,~\mathrm{CI} = [-0.986,~0.998]$}\DIFaddend ). We also looked for
...
(Fig.~\ref{fig:clustering-scatterplots}E), this trend held overall (\DIFdelbegin \DIFdel{$r(179) =
0.924, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) =
0.916,~p < 0.001,~\mathrm{CI} = [0.893,~0.936]$}\DIFaddend ), for each condition
individually (all $r$s $\geq 0.822$, all $p$s $< 0.001$), and across conditions
(\DIFdelbegin \DIFdel{$r(4) = 0.964, p = 0.002$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = 0.964,~p = 0.002$}\DIFaddend ). For late lists
(Fig.~\ref{fig:clustering-scatterplots}F), the results were more variable
(overall: \DIFdelbegin \DIFdel{$r(179) = 0.348, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.348,~p < 0.001$}\DIFaddend ; all non-visual conditions: $r$s $\geq
0.382$, all $p$s $\leq 0.037$; color: \DIFdelbegin \DIFdel{$r(29) = 0.453, p = 0.011$}\DIFdelend \DIFaddbegin \DIFadd{$r(29) = 0.453,~p = 0.011$}\DIFaddend ; location: \DIFdelbegin \DIFdel{$
r(28) = 0.190, p = 0.314$}\DIFdelend \DIFaddbegin \DIFadd{$
r(28) = 0.190,~p = 0.314$}\DIFaddend ; across-conditions: \DIFdelbegin \DIFdel{$r(4) = -0.036, p = 0.945$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = -0.036,~p = 0.945$}\DIFaddend ).
...
feature clustering on early lists \DIFdelbegin \DIFdel{trended towards showing }\DIFdelend \DIFaddbegin \DIFadd{showed }\DIFaddend stronger temporal clustering on later
lists (overall: \DIFdelbegin \DIFdel{$r(179) = 0.301, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.464,~p < 0.001,~\mathrm{CI} = [0.321, 0.582]$}\DIFaddend ; for
individual conditions: all $r$s \DIFdelbegin \DIFdel{$\geq 0.297$}\DIFdelend \DIFaddbegin \DIFadd{$\geq 0.377$}\DIFaddend , all $p$s \DIFdelbegin \DIFdel{$\leq 0.111$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 0.040$}\DIFaddend ; across
conditions: \DIFdelbegin \DIFdel{$
r(4) = 0.107, p = 0.840$}\DIFdelend \DIFaddbegin \DIFadd{$ r(4) = 0.451,~p = 0.369,~\mathrm{CI} = [-0.986, 0.998]$}\DIFaddend ). And
participants who showed stronger temporal clustering on early lists trended
towards showing stronger feature clustering on later lists (overall: \DIFdelbegin \DIFdel{$r(179) = 0.579, p < 0.001$; }\DIFdelend \DIFaddbegin \DIFadd{$r(179) =
0.266,~p < 0.001,~\mathrm{CI} = [0.129, 0.396]$; for individual conditions: }\DIFaddend all
\DIFdelbegin \DIFdel{non-visual
conditions: }\DIFdelend $r$s \DIFdelbegin \DIFdel{$\geq 0.323$, all
$p$s $\leq 0.082$; visual conditions: $r$s
$\geq 0.089$}\DIFdelend \DIFaddbegin \DIFadd{$\geq 0.298$}\DIFaddend , all $p$s \DIFdelbegin \DIFdel{$\leq 0.632$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 0.110$}\DIFaddend ; across conditions: \DIFdelbegin \DIFdel{$r(4) = 0.916, p =
0.010$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = 0.064,~p =
0.903,~\mathrm{CI} = [-0.972,~0.965$}\DIFaddend ). Taken together, the results displayed in
...
appeared to carry over \DIFdelbegin \DIFdel{across }\DIFdelend \DIFaddbegin \DIFadd{to later }\DIFaddend lists, even when the items on \DIFaddbegin \DIFadd{those }\DIFaddend later lists
...
across participants and conditions: \DIFdelbegin \DIFdel{$r(179) = 0.592, p < 0.001$; non-visual
feature conditions: all $r$s $\geq 0.350$, all $p$s $\leq 0.058$; color: $r(29)
= -0.071, p = 0.704$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.591,~p < 0.001,~\mathrm{CI} =
...
-0.183,~p = 0.325,~\mathrm{CI} = [-0.562,~0.258]$}\DIFaddend ; location: \DIFdelbegin \DIFdel{$r(28) = 0.032, p = 0.868$}\DIFdelend \DIFaddbegin \DIFadd{$r(28) = 0.031, p
= 0.870,~\mathrm{CI} = [-0.240,~0.296]$}\DIFaddend ; across conditions: \DIFdelbegin \DIFdel{$r(4) = 0.934, p = 0.006$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = 0.942,~p =
0.005,~\mathrm{CI} = [0.442,~1.000]$}\DIFaddend ). Although participants tended to show
weaker feature clustering on late lists
(\DIFdelbegin \DIFdel{Fig.~\ref{fig:fingerprint-trajectories}}\DIFdelend \DIFaddbegin \DIFadd{Figs.~\ref{fig:fingerprints},~\ref{fig:barplots}}\DIFaddend ) on \textit{average}, the
...
a \DIFdelbegin \DIFdel{larger improvement in recall }\DIFdelend \DIFaddbegin \DIFadd{smaller decrease in recall on early versus late lists
}\DIFaddend (Fig.~\ref{fig:clustering-carryover}B; overall: \DIFdelbegin \DIFdel{$r(179) = 0.378, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.307,~p < 0.001,
\mathrm{CI} = [0.148,~0.469]$}\DIFaddend ; category: \DIFdelbegin \DIFdel{$r(28) = 0.419, p = 0.021$}\DIFdelend \DIFaddbegin \DIFadd{$r(28) = 0.350,~p = 0.058,~\mathrm{CI}
= [0.050,~0.642]$}\DIFaddend ; size: \DIFdelbegin \DIFdel{$r(28) = 0.737, p < 0.001$; non-semantic conditions: all $r$s $\leq 0.252$, all $p$s $\geq 0.179$;
}\DIFdelend \DIFaddbegin \DIFadd{$r(28) = 0.708,~p < 0.001,~\mathrm{CI} = [0.472,~
...
0.052,~p = 0.787,~\mathrm{CI} = [-0.307,~0.360]$; }\DIFaddend across conditions: \DIFdelbegin \DIFdel{$r(4) = 0.773, p = 0.072$)on late lists, relative to early lists}\DIFdelend \DIFaddbegin \DIFadd{$r(4) =
0.635,~p = 0.176,~\mathrm{CI} = [-0.924,~0.981]$)}\DIFaddend . Participants who exhibited
...
condition (Fig.~\ref{fig:clustering-carryover}C; overall: \DIFdelbegin \DIFdel{$r(179) = 0.434, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(179) = 0.426,~p <
0.001,~\mathrm{CI} = [0.285,~0.544]$}\DIFaddend ; category: \DIFdelbegin \DIFdel{$r(28) = 0.229, p = 0.223$}\DIFdelend \DIFaddbegin \DIFadd{$r(28) = 0.110,~p = 0.564,~
\mathrm{CI} = [-0.284,~0.442]$}\DIFaddend ; all non-category conditions: all $r$s \DIFdelbegin \DIFdel{$\geq 0.448$}\DIFdelend \DIFaddbegin \DIFadd{$\geq
0.406$}\DIFaddend , all $p$s \DIFdelbegin \DIFdel{$\leq 0.012$}\DIFdelend \DIFaddbegin \DIFadd{$\leq 0.023$}\DIFaddend ; across conditions: \DIFdelbegin \DIFdel{$r(4) = 0.598, p = 0.210$}\DIFdelend \DIFaddbegin \DIFadd{$r(4) = 0.649,~p = 0.163,
\mathrm{CI} = [-0.856, 0.988]$}\DIFaddend ).
...
\begin{figure}
    ...
    \includegraphics[width=\textwidth]{figures/adaptive_results}
        ...
        of recall (taken across words, lists, and participants) for \DIFdelbeginFL \DIFdelFL{lists from
        }\DIFdelendFL each \DIFaddbeginFL \DIFaddFL{batch of
        four lists in the }\DIFaddendFL adaptive condition. \textbf{B.} Average temporal clustering scores
        for \DIFdelbeginFL \DIFdelFL{lists from }\DIFdelendFL each \DIFdelbeginFL \DIFdelFL{adaptive condition}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{batch of lists}\DIFaddendFL . \textbf{C.} Recall probability
        versus temporal clustering scores by participant (main panel; each
        participant contributes one dot per \DIFdelbeginFL \DIFdelFL{condition}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{batch}\DIFaddendFL ) and averaged within
        \DIFdelbeginFL \DIFdelFL{condition }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{batch }\DIFaddendFL (inset; each dot represents a single \DIFdelbeginFL \DIFdelFL{condition}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{batch}\DIFaddendFL ). \textbf{D.}
        ...
        All panels: Colors denote the sorting \DIFdelbeginFL \DIFdelFL{type }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{policy }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{condition}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{batch}\DIFaddendFL ) for each list.
        ...
        performance during the adaptive \DIFdelbeginFL \DIFdelFL{conditions}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{condition}\DIFaddendFL , see
...
stabilize lists relative to words on both random (\DIFdelbegin \DIFdel{$t(59) = 1.740, p = 0.087$}\DIFdelend \DIFaddbegin \DIFadd{$t(59) = 1.740,~p = 0.087, d
= 0.095,~\mathrm{CI} = [-0.187,~3.761]$}\DIFaddend ) and destabilize (\DIFdelbegin \DIFdel{$t(59) = 1.714, p = 0.092$}\DIFdelend \DIFaddbegin \DIFadd{$t(59) = 1.714,~p =
0.092,~d = 0.114,~\mathrm{CI} = [-0.351,~4.108]$}\DIFaddend ) lists
(Fig.~\ref{fig:adaptive}A). Participants showed no reliable differences in
their memory performance on destabilize versus random lists (\DIFdelbegin \DIFdel{$t(59) = -0.249, p = 0.804$}\DIFdelend \DIFaddbegin \DIFadd{$t(59) = -0.249, p
= 0.804,~d = -0.017,~\mathrm{CI} = [-2.327,~1.578]$}\DIFaddend ). Participants also
exhibited stronger temporal clustering on stabilize lists, relative to random
(\DIFdelbegin \DIFdel{$t(59) = 3.554, p = 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$t(59) = 3.428,~p = 0.001,~d = 0.306,~\mathrm{CI} = [1.635,~5.460]$}\DIFaddend ) and
destabilize (\DIFdelbegin \DIFdel{$t(59) = 4.045, p <
0.001$}\DIFdelend \DIFaddbegin \DIFadd{$t(59) = 4.174,~p < 0.001,~d = 0.374,~\mathrm{CI} = [1.964,~
6.968]$}\DIFaddend ) lists (Fig.~\ref{fig:adaptive}B). We found no reliable differences in
temporal clustering for items on random versus destabilize lists (\DIFdelbegin \DIFdel{$t(59) =
-0.781, p = 0.438$}\DIFdelend \DIFaddbegin \DIFadd{$t(59) =
-0.880,~p = 0.382,~d = -0.081,~\mathrm{CI} = [-3.165,~1.127]$}\DIFaddend ).
...
across all list types (\DIFdelbegin \DIFdel{$r(178) = 0.721, p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$r(178) = 0.701,~p < 0.001,~\mathrm{CI} = [0.590,~
0.789]$}\DIFaddend ), and for each list type individually (all $r$s \DIFdelbegin \DIFdel{$\geq 0.683$}\DIFdelend \DIFaddbegin \DIFadd{$\geq 0.651$}\DIFaddend , all $p$s
\DIFdelbegin \DIFdel{$\leq 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$< 0.001$}\DIFaddend ). Taken together, the results from the adaptive condition suggest
...
\caption{\textbf{\DIFaddFL{Ranked clustering scores versus ``chance'' for participants in
the feature-rich condition.}} \DIFaddFL{For each participant, we sorted their clustering
...
\DIFadd{We note that the multivariate aspect of the adaptive condition (i.e., sorting
...
}\textit{\DIFadd{specific}} \DIFadd{feature dimensions participants tended to cluster along
...
% specific findings:
%DIF <    reduced (early/late) vs. feature rich and reduced: event boundaries
%DIF >    reduced (early/late) vs. feature-rich and reduced: event boundaries
...
%DIF >      - richness of encoding
%     - context effects: role of irrelevant features, temporal context and order effects
%     - priming: being "primed" to
%     - situation models:

\DIFaddbegin \subsection*{\DIFadd{Memory consequences of feature variability}}

\DIFadd{Several prior studies have examined how varying the richness or experiences, or
the extensive of encoding, can affect memory. Although specific details
differ~\mbox{%DIFAUXCMD
\citep{BoniEtal22}}\hskip0pt%DIFAUXCMD
, in general these studies have found that richer and
more deeply or extensively encoded experiences are remembered
better~\mbox{%DIFAUXCMD
\citep{HargEtal12, Mada21, MeinEtal20}}\hskip0pt%DIFAUXCMD
. Our findings help to elucidate
...
held constant) only when those feature-rich lists are presented }\textit{\DIFadd{after}}
\DIFadd{reduced lists suggests that some factors that influence the richness or depth
of encoding may be relative, rather than absolute. In other words,
}\textit{\DIFadd{increases}} \DIFadd{in richness (e.g., relative to a recency-weighted baseline)
may be more important than the absolute complexity or numbers of meaningful
features.
}

\DIFadd{Some prior studies have suggested that people can ``cue'' their memories using
...
with the contexts associated with other items in memory~\mbox{%DIFAUXCMD
\citep{Kaha20}}\hskip0pt%DIFAUXCMD
. Since
context is the defining hallmark of episodic memory~\mbox{%DIFAUXCMD
\citep{Tulv83}}\hskip0pt%DIFAUXCMD
,
...
other dimensions, such as semantic similarity~\mbox{%DIFAUXCMD
\citep{DavaEtal03, SochEtal09}}\hskip0pt%DIFAUXCMD
.
...
may be some additional factors at play with respect to the }\textit{\DIFadd{dynamics}} \DIFadd{of
...
\DIFadd{We did }\textit{\DIFadd{not}} \DIFadd{find any evidence that changing words' appearances
}\textit{\DIFadd{harmed}} \DIFadd{memory performance, e.g., by distracting them with irrelevant
information~\mbox{%DIFAUXCMD
\citep{MarsEtal15, Lang05, MarsEtal12, ReinEtal92}}\hskip0pt%DIFAUXCMD
. Nor did we find
any evidence that }\textit{\DIFadd{changes}} \DIFadd{in the presence of potentially
...
\DIFaddend \subsection*{Context effects on memory performance and organization}
...
sounds~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{BeamJone10, SahaSmit14}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{BeamJone98, SahaSmit14}}\hskip0pt%DIFAUXCMD
...
\DIFaddbegin \subsection*{\DIFadd{Free recall of blocked versus random categorized word lists}}

\DIFadd{A large number of prior studies have compared participants' memories for
...
lists~\mbox{%DIFAUXCMD
\citep{CofeEtal66, DAgo69, Dall64, Shap70, ShapPale70, Kint70,
BoweEtal69, LuekEtal71, Puff74}}\hskip0pt%DIFAUXCMD
. Other studies suggest that these order
effects may also be modulated by factors like list length and the numbers of
exemplars in each category~\mbox{%DIFAUXCMD
\citep[e.g.,][]{BorgMang72}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{Although we did not directly manipulate ``blocking'' in our order manipulation
conditions, our sorting procedures in those conditions (see
}\textit{\DIFadd{Constructing feature-sorted lists}}\DIFadd{) have }\textit{\DIFadd{indirect}} \DIFadd{effects on
...
consequences for memory~\mbox{%DIFAUXCMD
\citep{RadvZack17, DuBrEtal17, DuBrDava16, DuBrDava13}}\hskip0pt%DIFAUXCMD
.
}
...
\DIFaddbegin \subsection*{\DIFadd{How do different types of clustering relate to each other, and to memory performance?}}

\DIFadd{When the words on a studied list are presented in a random order, different types
...
semantic clustering, and so on~\mbox{%DIFAUXCMD
\citep{HealUitv19, HowaKaha02b, SedeEtal10}}\hskip0pt%DIFAUXCMD
.  Further,
...
memory performance~\mbox{%DIFAUXCMD
\citep{SedeEtal10}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{The notion of ``multiple pathways to recall'' discussed above (see
}\textit{\DIFadd{Memory consequences of feature variability}}\DIFadd{) suggests one potential
...
recall~\mbox{%DIFAUXCMD
\citep{SochEtal09}}\hskip0pt%DIFAUXCMD
. Meanwhile, extra-list intrusion errors (i.e., false
``recalls'' of items that were never encountered on the list) often tend to
share semantic features with recently recalled items~\mbox{%DIFAUXCMD
\citep{ZaroEtal06} }\hskip0pt%DIFAUXCMD
and
also often lead the participant to stop recalling additional
items~\mbox{%DIFAUXCMD
\citep{MillEtal12a}}\hskip0pt%DIFAUXCMD
. Speculatively, over-reliance on semantic cues may lead to
more intrusion errors, which in turn may lead to fewer recalls overall.
}

\DIFadd{Our findings extend these prior results to consider lists that are }\textit{\DIFadd{not}}
\DIFadd{ordered randomly. Because ordering the words on a list along a particular
...
clustering }\textit{\DIFadd{also}} \DIFadd{show strong temporal clustering on ordered lists
...
lists (also see }\textit{\DIFadd{Factoring out the effects of temporal clustering}}\DIFadd{).
...
Conceptualization: JRM and ACH. \DIFdelbegin \DIFdel{Methodology: JRMand ACH. Software: JRM}\DIFdelend \DIFaddbegin \DIFadd{Data curation: JRM}\DIFaddend , PCF, \DIFdelbegin \DIFdel{CEF, and ACH. Analysis}\DIFdelend \DIFaddbegin \DIFadd{ACH. Formal analysis}\DIFaddend :
JRM, PCF, and ACH. \DIFdelbegin \DIFdel{Data collection: }\DIFdelend \DIFaddbegin \DIFadd{Funding acquisition: JRM. Investigation: }\DIFaddend ECW, PCF, MRL, AMF,
BJB, DR, \DIFdelbegin \DIFdel{and CEF. Data curation and management: ECW
}\DIFdelend \DIFaddbegin \DIFadd{CEF, and ACH. Methodology: JRM and ACH. Project administration: ECW
and PCF. Resources: JRM. Software: JRM}\DIFaddend , PCF, \DIFdelbegin \DIFdel{MRL}\DIFdelend \DIFaddbegin \DIFadd{CEF}\DIFaddend , and ACH. \DIFaddbegin \DIFadd{Supervision: JRM and
ACH. Validation: JRM, PCF, and ACH. }\DIFaddend Writing (original draft): JRM. Writing
(review and editing): ECW, PCF, MRL, AMF, BJB, DR, CEF, and ACH.
\DIFdelbegin \DIFdel{Supervision: JRM and ACH. Project administration: ECW and
PCF. Funding acquisition: JRM.
}\DIFdelend 

\subsection*{\DIFdelbegin \DIFdel{Data and code availability}\DIFdelend \DIFaddbegin \DIFadd{Author note}\DIFaddend }
...
https://github.com/ContextLab/adaptiveFR. We have also released an associated
Python toolbox for analyzing free recall data, which may be found at
https://cdl-quail.read\-the\-docs.io/\-en/\-latest\DIFdelbegin \DIFdel{/}\DIFdelend \DIFaddbegin \DIFadd{.  Note that this study was not
...
and the Context and Episodic Memory Symposium (2017)}\DIFaddend .
