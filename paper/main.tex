...
unique categories, each with exactly 4 exemplars from each category. Second, we
...
and at random from the interval $\left[0, 255\right]$, corresponding to the red (r),
...
conditions served as ``anchor points.'' In the first anchor point condition,
...
list. In the second anchor point condition, which we call the \textit{reduced}
...
from the uniform distribution bounded between 0 and 1 (inclusive). The identity
...
conditions, we concatenated the full set of fingerprints (across all list
groupings, participants, and experimental conditions) to create a large matrix
with number-of-list-groupings $\times$ number-of-participants rows and seven
columns (one for each feature clustering score, plus an additional temporal
...
two-dimensional coordinate for each \textit{list group}, rather than for each
individual list. We used these embeddings solely for visualization. All
...
manipulation conditions was indistinguishable from performance on the early
...
conditions. Since results were similar across both semantic conditions
...
In general, participants' memories can be influenced by a wide range of
positive and negative factors~\citep{BaloEtal92, ClayChat89, Donn88,
FlexTulv82, GottEtal12, HuanEtal04, Hube08, HubeEtal01, McNa94, Neel77, Rabi86,
TulvScha91, WatkEtal92, WiggMart98}.
...
sounds we are likely to hear, etc.). Similarly, we might learn a schema
describing how events are likely to unfold \textit{across} any sit-down
restaurant-- e.g., open the door, wait to be seated, receive a menu, decide
what to order, place the order, and so on. Situation models and schemas can
help us to generalize across our experiences, and to generate expectations
about how new experiences are likely to unfold. When those expectations are
violated, we can perceive ourselves to have crossed into a new situation.
...
are studied, can lead participants to behave similarly to what one might expect
upon crossing an event boundary. Adding in variability in font color and
...
those later lists. (We found the same pattern when we varied words' colors and
locations on early lists but held them constant on late lists.) One potential
explanation is that participants perceive an ``event boundary'' to have
occurred when they encounter the first ``late'' list. According to contextual
change accounts of memory across event boundaries~\citep[e.g.,][]{SahaKell02},
...
versus naturalistic stimuli (e.g., real-world experiences, narratives, films,
etc.)~\citep{NastEtal20, HeusEtal21, Mann21a, LeeEtal20}. One reason is that
...
lists used in laboratory memory tasks are most often ordered randomly, by design,
to \textit{remove} meaningful temporal structure in the
...
stimulus, or from idiosyncratic interactions or interference between our memory
systems and the stimulus. This challenge implies that, to understand the
fundamental nature of memory in its ``pure'' form, we should study memory for
highly simplified stimuli that can provide relatively unbiased (compared with
real-world experiences) measures of the relevant patterns and tendencies.

The experiment we report in this paper was designed to help bridge some of the
...
Our findings indicate that the way word lists are presented affects how people
...
Conceptualization: JRM and ACH. Methodology: JRM and ACH. Software: JRM, KZ,
PCF, CEF, and ACH. Analysis: JRM, PCF, and ACH. Data collection: KZ, EW, PCF,
MRL, AMF, BJB, and CEF. Data curation and management: KZ, EW, PCF, MRL, ACH.
Writing (original draft): JRM. Writing (review and editing): KZ, EW, PCF, MRL,
AMF, BJB, CEF, and ACH. Supervision: JRM and ACH. Project administration: KZ,
EW, and PCF. Funding acquisition: JRM.
...
https://github.com/Context\-Lab/efficient-learning-code. Code for running the
...
We acknowledge useful discussions, assistance in setting up an earlier version
of this study, and assistance with some of the data collection efforts from
Rachel Chacko, Joseph Finkelstein, Sheherzad Mohydin, Lucy Owen, Gal Perlman,
Darya Romanova, Jake Rost, Jessica Tin, Marisol Tracy, and Peter Tran. Our work
was supported in part by NSF CAREER Award Number 2145172 to JRM. The content
