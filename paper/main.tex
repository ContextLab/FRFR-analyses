...
condition, the words' colors and locations varied randomly, and in the reduced
condition, words were always presented in black, at the center of the display.
Aggregating across all lists for each participant, we found no difference in
recall accuracy (i.e., the proportions of words successfully recalled) for
feature-rich versus reduced lists ($t(126) = -0.290,~p = 0.772$, Cohen's~$d =
-0.051$, bootstrap-estimated 95\% confidence interval (CI)~$= [-2.387,~
...
14.386]$; semantic category clustering: $t(126) = 10.148,~p < 0.001,~d =
1.796,~ \mathrm{CI} = [7.324,~13.778]$; size clustering: $t(126) = 12.033,~p <
0.001,~ d = 2.129,~\mathrm{CI} = [9.030,~15.918]$; word length clustering:
$t(126) = 10.720,~p < 0.001,~d = 1.897,~\mathrm{CI} = [7.442,~15.174]$; first
letter clustering: $t(126) = 6.679,~p < 0.001,~d = 1.182,~\mathrm{CI} =
[4.490,~ 9.611]$; see \textit{Permutation-corrected feature clustering scores}
...
those new features are independent of the words themselves and vary randomly
across words. We found no evidence that those additional uninformative features
were distracting (in terms of their impact on memory performance), but they did
...
surprise, participants in \textit{neither} condition exhibited reliable early-versus-late
differences in accuracy (reduced (early): $t(41) = 1.499, ~p =
...
the same blends of features (accuracy for feature-rich vs.~reduced (early):
$t(107) = -2.230,~p = 0.028,~d = -0.439,~\mathrm{CI} = [-4.252,~-0.229]$;
reduced vs.~reduced (early): $t(101) = -2.045,~p = 0.043,~d = -0.410,~
...
did not appear to impact recall performance (accuracy for feature-rich vs.~
reduced (late): $t(106) = -0.638,~p = 0.525,~d = -0.126,~\mathrm{CI} = [-2.720,~
1.362]$; reduced vs.~reduced (late): $t(100) = -0.407,~p = 0.685,~d =
...
removing features that had initially been present versus adding features
that had initially been absent. In contrast, we found that
...
feature-rich vs.~reduced lists are reported above; temporal clustering in
reduced vs.~reduced (early) and reduced vs.~reduced (late) conditions:
...
conditions (temporal clustering in feature-rich vs.~reduced (early) and
feature-rich vs.~reduced (late) conditions: $t$s $\geq -1.379$,~$p$s $\geq
0.171$; feature-based clustering in feature-rich vs.~reduced (early) and
feature-rich vs.~reduced (late) conditions: $|t|$s $\leq 1.441$,~$p$s $\geq
0.153$).
...
(vs.~early feature-rich lists; category: $t(95) = 8.813,~p < 0.001,~d =
...
clustering, and so on). Although it is not possible to fully separate feature-based
...
lists that were sorted semantically better than those sorted lexicographically
...
temporally clustered semantically sorted lists more strongly than lists sorted either
lexicographically ($t(118) = 5.620,~p < 0.001,~d = 1.026,~\mathrm{CI} = [3.486,~
8.010]$) or visually ($t(119) = 6.613,~p < 0.001,~d = 1.202,~\mathrm{CI} =
[4.481,~9.464]$), but did not show reliable differences in
