...
holding the font color fixed at black, and the words' locations fixed
at the center of the screen), we compared participants' performance on the
...
condition, the words' colors and locations varied randomly, and in
...
difference in recall accuracy (i.e., the proportions of words successfully recalled)
for feature-rich versus reduced lists ($t(126) = -0.290,~p = 0.772$,
Cohen's~$d = -0.051$, bootstrap-estimated 95\% confidence interval (CI)~$= [-2.387,~
...
features are independent of the words themselves and
...
fingerprints are relatively stable across lists, and that each participant's fingerprint
is unique to them.
...
presented words ($z$-scored within condition) computed based on each condition's
feature of focus, and plotted as a function of presentation position. See
\textit{Defining feature-based distances} for additional information. \textbf{C.} Proportion of
...
\begin{figure}[tp] \centering
    ...
    (across-participant) average memory fingerprint for a single list. Order
    ...
    the bottom panels display the Euclidean distances between each per-list memory
